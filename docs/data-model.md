...
Nine models carry a non-nullable `orgId String` FK to `Organization.id` (cascade-delete from the org). These are Conduit's business data; everything else is Better Auth or the audit log.

| Model | What it holds | `orgId` source |
|---|---|---|
| `Workflow` | Top-level user config. `definition` JSON holds triggers/nodes/edges/mcpServers/ui; `webhookSecret` + `temporalSlug` live here too. | stamped on create |
