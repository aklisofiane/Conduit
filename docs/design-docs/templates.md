...
| `templates/review.json` | 1 | Polling on `pull_requests` filtered by `label = "conduit-review"` (no `pr_state` filter — fires on draft PRs too) → `Review` (`code-reviewer` preset, no MCP) evaluates the PR diff against base and writes a per-file verdict + `APPROVE`/`REQUEST_CHANGES_AGENT`/`REQUEST_CHANGES_HUMAN` classification to `.conduit/` → `Publish` (`publish` preset, GitHub MCP) submits a self-contained summary review and routes one of three outcomes in prose (so GitLab works too): **all good** → `conduit-merge` + mark ready (→ `merge.json`); **agent-fixable** → set draft + re-apply `conduit-dev` to the issue (→ loops back to `develop.json`); **needs human** → set draft + park the issue with the `Review` status/label. Every outcome removes `conduit-review` from **both** the PR and the issue. A loop guard caps agent-fix retries at two (counted from the reused PR's changes-requested history) before forcing the human-park. Pairs with `develop.json` and `merge.json` as a label-gated pipeline. |
