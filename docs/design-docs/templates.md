...
The auto-chain: `analyze`'s Publish branches on the analysis. A clear analysis applies `conduit-dev` + status `In Progress`, so an opened issue flows to a draft PR untouched. If open questions remain for the user, it instead applies the **`conduit-human-review` label** and sets a human-facing `Review` status, then parks the issue — `conduit-human-review` is a writeback target that signals "parked for a human," never a trigger value, so it gates no workflow. GitHub Projects users who prefer drag-to-column can swap `develop`'s trigger filter to `status=Dev` (a documented per-template knob — "rename the trigger filter to match your board's column or label names"). GitLab boards are label-native, so the label default works out of the box. The trigger engine treats `label` and `status` as equal filter primitives (`packages/shared/src/trigger/`) — this is template configuration, not an engine feature.

**Cross-platform human signals.** The human-park signal is carried by the `conduit-human-review` label (renders on a GitLab label board) plus the `Review` board status (renders on a GitHub Projects board), so the "needs user review" park is visible whichever board system the platform uses. Using a real `conduit-*` label here — rather than a same-named "Review" label split across `Review`/`review` casing — collapses the earlier ambiguity into one canonical value. The board "Review" *status* is independent and left untouched.

### Conduit's labels and fix-forward creation

The four `conduit-*` labels Conduit's own workflows use live in a single registry, [`packages/shared/src/label/`](../../packages/shared/src/label/index.ts) (`CONDUIT_LABELS` + `isConduitLabel` / `getConduitLabel`): three AI-handoff gates (`conduit-dev`, `conduit-review`, `conduit-merge`) and the `conduit-human-review` park target. Template JSON keeps its own literal label strings — templates are standalone, version-controlled artifacts, not refactored to import code — so the registry is what the *app* reads, not the templates.

A label-gated trigger or a writeback target only fires/lands if the label exists on the repo/project. Rather than require hand-creating them on GitHub/GitLab, Conduit makes every Conduit label fixable in place and preventable at connect time:

- **Prevent at connection time.** Creating a new repo/project connection offers a post-create prompt to add the four registry labels (per-label outcome shown). See [connections.md](./connections.md).
- **Fix forward in the trigger panel.** An unmatched `conduit-*` value in a trigger's label filter renders an inline "create label" action instead of dead "(not found)" text; on success the labels query is invalidated. Non-Conduit unmatched values keep the plain text.

Both affordances call `POST /trigger/ensure-labels` (idempotent create, GitHub + GitLab — see [ARCHITECTURE.md](../ARCHITECTURE.md#trigger-config-time-helpers)). Colors/descriptions come from the registry; non-canonical names fall back to neutral gray.
...
|---|---|---|
| `templates/analyze.json` | 1 | Polling on `status = "Todo"` → `Research` (GitHub MCP) → `Review` → `Publish` (GitHub MCP) updates the issue body with a marker-bracketed analysis section, then branches: clear analysis → `conduit-dev` + status `In Progress` (auto-chains into `develop.json`); open questions remain → `conduit-human-review` label + human-facing `Review` status (parks for the user). Status-gated entry (human gesture) → label-gated handoff. Uses `research` / `plan-reviewer` / `publish` presets. |
| `templates/develop.json` | 1 | Polling on `label = "conduit-dev"` (applied by Analyze, or by a human) → `Planner` (`planner` preset) fans out to `Dev` (`developer` preset) + `Tests` (`tests` preset) → `Docs` (`docs` preset) on branched worktrees → merge-back → `QA` (`qa` preset) reuses or opens a draft PR and swaps the issue label `conduit-dev`→`conduit-review` (also applies `conduit-review` to the PR in prose; status untouched). On a loop-back from Review the Planner reads the linked PR's latest changes-requested review and scopes the fix. GitHub board users can swap the trigger to `status=Dev`. |
| `templates/review.json` | 1 | Polling on `pull_requests` filtered by `label = "conduit-review"` (no `pr_state` filter — fires on draft PRs too) → `Review` (`code-reviewer` preset, no MCP) evaluates the PR diff against base and writes a per-file verdict + `APPROVE`/`REQUEST_CHANGES_AGENT`/`REQUEST_CHANGES_HUMAN` classification to `.conduit/` → `Publish` (`publish` preset, GitHub MCP) submits a self-contained summary review and routes one of three outcomes in prose (so GitLab works too): **all good** → `conduit-merge` + mark ready (→ `merge.json`); **agent-fixable** → set draft + re-apply `conduit-dev` to the issue (→ loops back to `develop.json`); **needs human** → set draft + park the issue with the `conduit-human-review` label and `Review` status. Every outcome removes `conduit-review` from **both** the PR and the issue. A loop guard caps agent-fix retries at two (counted from the reused PR's changes-requested history) before forcing the human-park. Pairs with `develop.json` and `merge.json` as a label-gated pipeline. |
...
| `templates/nightly-review.json` | 1 | `cron` trigger (`0 2 * * *` UTC, `branch: main`; depends on the `cron` trigger type) → `Scope` (`scope` preset) identifies changed files and writes a per-domain manifest → fans out to four parallel reviewers `Security` / `Quality` / `Refactor` / `Performance` (all `code-analyst` preset, one domain each) → `Publisher` (`issue-publisher` preset, GitHub MCP) opens one issue per finding, routed by confidence to the `conduit-dev` label (auto-implement) or the `Review` status + `conduit-human-review` label (park for a human) via `issueWriteback`. Six agents; not a label-gated stage — a standalone scheduled fan-out. |
