...
| 6 — Workflow templates | `/templates/*.json` catalog + `GET /api/templates` (boot-validated); multi-workflow bundle schema in `@conduit/shared/template`; `POST /api/workflows/from-template/:templateId` (atomic `$transaction`, placeholder resolution, per-workflow validation, post-commit schedule upsert); "From template" picker + connection-binding UI; v1 templates (`analyze`, `develop`, `pr-review`, `review`, `nightly-review`). See [templates.md](./design-docs/templates.md). | `test/e2e/phase6-template-run.test.ts` |
