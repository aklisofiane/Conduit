...
| `apps/worker` | Temporal TS SDK | Executes `agentWorkflow` — loads nodes, topo sorts, invokes the orchestrator activity per node. The activity spawns a fresh `agent-runner` container, writes a `RunnerRequest` to its stdin, and translates each returned `RunnerEvent` back into Prisma writes + Redis publishes + Temporal heartbeats. Also executes `pollWorkflow` → `pollBoardActivity` when a Temporal Schedule fires — dispatches on the trigger's `type` and `boardConnectionId` presence (board issues, repo issues, or repo PRs), diffs the matching set against `PollSnapshot`, and starts `agentWorkflow`s for new matches. GitHub polling uses two-phase fetching: lightweight metadata first, then batched body hydration (via `nodes(ids:)`) only for items that survive dedup and filter gates. Issue/PR bodies are capped at 64 KB in both polling and webhook paths (`capTriggerBody`). `cronWorkflow` handles cron triggers — one tick per schedule fire, builds a `mode: 'scheduled'` event, starts the agent workflow. `repoAnalysisWorkflow` orchestrates repo analysis (dynamic fan-out over discovered components, reusing `runAgentNode` via the analysis adapter; see [design-docs/repo-analysis.md](./design-docs/repo-analysis.md)). **Provider SDKs no longer live here** — they're baked into the runner image |
...
2. **Event normalized** → platform-specific mapper produces a `TriggerEvent` (stable shape across all platforms). Issue/PR bodies are capped at 64 KB (`capTriggerBody`) during normalization. Polling triggers skip steps 1–2 — `pollBoardActivity` synthesizes the `TriggerEvent` from a lightweight metadata fetch, then hydrates bodies only for items that pass filtering. Cron triggers skip 1–3 — the activity builds a `mode: 'scheduled'` event with `repo` but no `issue`/`pr`.
...
| `POST` | `/connections/:id/analyze` | Start a component-based review-workflow analysis of the connection's repo. Mints a hidden internal run + `RepoAnalysis` row and starts `repoAnalysisWorkflow`. 400 if not repo-scoped, 409 if one's already in progress (per-connection advisory lock). See [design-docs/repo-analysis.md](./design-docs/repo-analysis.md). |
| `GET` | `/connections/:id/analysis` | Latest analysis for the connection — `status`/`phase`, the suggestion `resultBundle`, and `droppedComponents`. Drives the badge, progress card, and suggestions gallery. |
...
| `POST` | `/trigger/list-branches` | Body: `{ connectionId }`. Same binding lookup; branches on `binding.platform` — `github_repo` → `listRepoBranches`, `gitlab_project` → `listGitlabProjectBranches` (a non-repo/project scope is a 400). Returns `string[]` of remote branch names (paginated). Drives the branch combobox in the **Cron** trigger panel — see [FRONTEND.md](./FRONTEND.md). The web side calls it via `useRepoBranches`. |
| `POST` | `/trigger/ensure-labels` | Body: `{ connectionId, names: string[] }`. Same binding lookup; branches on `binding.platform` — `github_repo` → `createRepoLabel`, `gitlab_project` → `createGitlabProjectLabel` (a non-repo/project scope is a 400). Idempotently creates each label (GitHub `422 already_exists` / GitLab `409` treated as success; never overwrites). Returns one `{ name, status: 'created' \| 'exists' \| 'failed', error? }` per requested name — a per-label failure (e.g. a read-only token) doesn't fail the call, so callers can show partial success. Colors/descriptions come from the `@conduit/shared/label` registry, falling back to neutral gray for non-canonical names. Backs the connection post-create label prompt and the trigger panel's inline "create label" action — see [templates.md > Conduit's labels and fix-forward creation](./design-docs/templates.md#conduits-labels-and-fix-forward-creation). |
...
| `POST` | `/workflows/import` | Instantiate a **user-uploaded** bundle instead of a catalog id. Body: `{ template, bindings }` — the bundle is parsed against the same `templateFileSchema` the disk loader uses, then fed through the identical instantiate core as `from-template` (server re-derives placeholders, ignoring any client summary). Counterpart to client-side workflow export. See [design-docs/templates.md](./design-docs/templates.md#export-and-import). |
...
- **Domain subpath exports from `@conduit/shared`** — consumers import `@conduit/shared/agent`, `/trigger`, `/mcp`, `/workflow`, `/runtime`, `/temporal`, `/workspace`, `/skill`, `/platform`, `/analysis` rather than a single barrel, so each app only pulls the schemas it actually uses. The root barrel re-exports those subpaths for convenience; **`/crypto` and `/webhook` are deliberately *not* in the root barrel** — they pull `node:crypto`, which Vite would otherwise drag into the web bundle. Backend-only code imports them via the narrow subpath.
