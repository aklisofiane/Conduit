...
| Cron tick fires while a previous cron run is still executing | Schedule `overlap = SKIP` drops the tick silently. No `TicketLock` — overlap policy is the only concurrency gate. |
| Concurrent triggers on the same workflow + ticket, while a run is in flight | Silently dropped at the Temporal boundary. Workflow ID `run-<workflowId>-<ticketId>` (plus an optional cosmetic slug prefix — see [temporal-id-slug.md](./design-docs/temporal-id-slug.md)) collides with the in-flight run, Temporal throws `WorkflowExecutionAlreadyStarted`, the trigger handler catches it and swallows the trigger (webhook handler returns 200 so the platform doesn't retry; poll-loop skip is internal). No `WorkflowRun` row created. After termination, a new trigger on the same ticket starts a fresh run (`WorkflowIdReusePolicy = ALLOW_DUPLICATE`) — this is how board cycles (Dev → Review → Dev) keep re-firing. See [branch-management.md](./design-docs/branch-management.md#concurrency). |
...
| Webhook storms / redundant queued runs | Known gap. Polling set-diff dedup collapses most of this; Temporal workflow-ID uniqueness catches the rest. Beyond that, backpressure and storm collapse are deferred to a later phase. |
