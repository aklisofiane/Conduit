...
describe('paths', () => {
  ...
  describe('baseClonePath', () => {
    it('produces <root>/<platform>/<host>/<owner>/<repo>.git for a cloud host', () => {
      ...
      expect(p).toBe(
        path.join('/tmp/conduit-test-paths', 'base-clones', 'github', 'github.com', 'acme', 'api.git'),
      );
    });
    ...
    it('produces <root>/<platform>/<host>/<owner>/<repo>.git for gitlab cloud', () => {
      ...
      expect(p).toBe(
        path.join('/tmp/conduit-test-paths', 'base-clones', 'gitlab', 'gitlab.com', 'acme', 'frontend.git'),
      );
    });
    ...
    it('handles self-hosted hosts without port', () => {
      ...
      expect(p).toBe(
        path.join('/tmp/conduit-test-paths', 'base-clones', 'github', 'ghe.example.com', 'acme', 'api.git'),
      );
    });
    ...
    it('handles self-hosted hosts with port', () => {
      ...
      expect(p).toBe(
        path.join('/tmp/conduit-test-paths', 'base-clones', 'gitlab', 'gitlab.acme.io:8443', 'ops', 'deploy.git'),
      );
    });
